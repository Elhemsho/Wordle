...
function startChallengePoll() {
  ...
  if (!state.currentUser) return;
  challengePollInterval = setInterval(pollChallenges, 4000);
}
...
async function sendChallenge(friendId, friendUsername) {
  ...
  try {
    ...
    activeChallengeOpponent = { id: friendId, username: friendUsername };
    showToast(de ? `Herausforderung an ${friendUsername} gesendet! ⚔️` : `Challenge sent to ${friendUsername}! ⚔️`, 'success', 4000);
    showChallengeWaitingOverlay(friendUsername);
  } catch(e) {
    showToast(de ? 'Fehler beim Senden.' : 'Error sending challenge.', 'error');
  }
}
...
async function onChallengeReceived(challenge) {
  ...
  setEl('challenge-decline-btn', de ? 'Ablehnen ✗' : 'Decline ✗');
  overlay.dataset.challengeId = challenge.id;
  overlay.dataset.fromUser = challenge.from_user;
  overlay.dataset.fromName = fromName;
  overlay.classList.add('open');
}
...
async function acceptChallenge() {
  ...
  const fromName = overlay.dataset.fromName;
  ...
  const waitPoll = setInterval(async () => {
    ...
    try {
      ...
      if (c.status === 'active') {
        ...
        showToast(de ? `${opponentName} hat abgelehnt.` : `${opponentName} declined.`, 'error', 4000);
      }
    } catch(e) {}
  }, 2000);
}
...
async function pollUntilBothDone() {
  const poll = setInterval(async () => {
    if (!activeChallengeId) { clearInterval(poll); return; }
    try {
      const rows = await sbFetch(`challenges?id=eq.${activeChallengeId}&select=*`);
      const c = rows?.[0];
      if (!c) { clearInterval(poll); return; }
      if (c.status === 'finished' || (c.p1_done && c.p2_done)) {
        clearInterval(poll);
        showChallengeResult(c);
      }
    } catch(e) {}
  }, 2000);
}
...
async function showChallengeResult(challenge) {
  ...
  const myAttempts = isP1 ? challenge.p1_attempts : challenge.p2_attempts;
  const myTime = isP1 ? challenge.p1_time : challenge.p2_time;
  const oppAttempts = isP1 ? challenge.p2_attempts : challenge.p1_attempts;
